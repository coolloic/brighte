...
- Migrations (`src/database/migrations/`) run in a transaction by default. For `CREATE INDEX CONCURRENTLY`, export `transaction = false` from the migration and `DROP INDEX CONCURRENTLY IF EXISTS` first.
