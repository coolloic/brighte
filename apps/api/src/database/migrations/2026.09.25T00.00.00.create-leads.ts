...
export const up: Migration = async ({ context: queryInterface }) => {
  ...
  await queryInterface.createTable('leads', {
    // UUID v7, generated by the app (Lead model): time-ordered, so inserts append to the index like
    // a serial, but not guessable and does not reveal how many leads exist. No column default:
    // Postgres 17 has no uuidv7(), so raw SQL inserts must supply the id.
