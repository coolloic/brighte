 ...
 */
export function FormField({ id, label, hint, error, required, className, type, ...inputProps }: FormFieldProps) {
  ...
  return (
    <div className={className}>
      ...
      {type === "password" ? (
        // Passwords get a show/hide toggle.
        <PasswordInput id={id} required={required} invalid={Boolean(error)} aria-describedby={describedBy} {...inputProps} />
      ) : (
        <Input id={id} type={type} required={required} invalid={Boolean(error)} aria-describedby={describedBy} {...inputProps} />
      )}
      {hint && (
