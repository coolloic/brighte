...
export const WithSelection: Story = {
  ...
  play: async ({ canvas, canvasElement }) => {
    ...
    // The selection must never take the badges' color: their background has to stay distinct.
    const badgeBackground = `rgb(${readToken("surface-brand")!.join(", ")})`;
    ...
    for (const container of [selectedRow, selectedRow.querySelector("th")!, selectedCard]) {
      await expect(getComputedStyle(container).backgroundColor).not.toBe(badgeBackground);
    }
    // Desktop marker: the green bar on the name cell.
    await expect(getComputedStyle(selectedRow.querySelector("th")!).boxShadow).toContain(`rgb(${readToken("action")!.join(", ")})`);
  },
};
