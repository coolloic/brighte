# Architecture

How Brighte Eats fits together: the database, the system, and the two main request flows. The diagrams are [Mermaid](https://mermaid.js.org), which GitHub renders in place; to edit one visually or export it for slides, paste it into [mermaid.live](https://mermaid.live) or draw.io (*Arrange → Insert → Advanced → Mermaid*).

## Database (ER diagram)

Postgres 17. The schema is owned by the migrations in `apps/api/src/database/migrations/` (see [Data modelling trade-offs](../README.md#data-modelling-trade-offs) for why it looks like this).

```mermaid
erDiagram
    LEADS ||--|{ LEAD_SERVICE_TYPES : "is interested in"
    SERVICE_TYPES ||--o{ LEAD_SERVICE_TYPES : "is chosen by"

    LEADS {
        uuid id PK "UUID v7, generated by the app"
        varchar(255) name "trimmed; the API accepts up to 70 characters"
        varchar(255) email UK "stored lowercase"
        varchar(20) mobile "normalised to 04xxxxxxxx"
        varchar(4) postcode "4 digits, leading zero kept"
        timestamptz createdAt
        timestamptz updatedAt
    }
    LEAD_SERVICE_TYPES {
        uuid leadId PK, FK "ON DELETE CASCADE"
        int serviceTypeId PK, FK "ON DELETE RESTRICT"
        timestamptz createdAt
    }
    SERVICE_TYPES {
        int id PK "serial"
        varchar(50) code UK "stable key, e.g. pick-up"
        varchar(255) label "display name, e.g. Pick-up"
        boolean active "default true; false = retired"
        timestamptz createdAt
        timestamptz updatedAt
    }
    USERS {
        int id PK "serial"
        varchar(255) email UK
        varchar(255) name
        enum role "USER or ADMIN, default USER"
        varchar(255) passwordHash "scrypt"
        timestamptz createdAt
        timestamptz updatedAt
    }
```

- **A lead has one or more services** (`||--|{`): at least one is required by the API's validation, and `register` writes the lead and its rows in one transaction. The database itself allows zero.
- **A service type can have no leads** (`||--o{`). Deleting one that leads use is refused (`RESTRICT`); retire it with `active = false` instead, so existing leads keep it. Deleting a lead deletes its rows in the join table (`CASCADE`).
- **`users` stands alone**: they're the admin (and user) accounts that sign in to the dashboard, not leads.
- `SequelizeMeta` (not shown) records which migrations have run.

**Indexes** (Mermaid can't draw them):

| Table | Index | Serves |
|---|---|---|
| `leads` | primary key `id` | `lead(id)` |
| `leads` | unique `email` | duplicate-lead check (`CONFLICT`), even under concurrent requests |
| `leads` | `(createdAt, id)` | the default newest-first order, with a stable tie-breaker for pages |
| `leads` | GIN trigram on `name`, `email` and `mobile` (`pg_trgm`) | dashboard search, which matches anywhere in the text |
| `leads` | `postcode` with `text_pattern_ops` | dashboard search's postcode prefix match; with the three above, every branch of the search's `OR` has an index |
| `lead_service_types` | primary key `(leadId, serviceTypeId)` | a lead's services; no duplicates |
| `lead_service_types` | `serviceTypeId` | filtering leads by service |
| `service_types` | unique `code` | looking types up by code |
| `users` | unique `email` | sign-in |

## System architecture

```mermaid
flowchart TB
    subgraph clients [" Browsers "]
        direction LR
        visitor["Visitor<br/>register form"]
        admin["Admin<br/>leads dashboard"]
    end

    subgraph web ["apps/web · Next.js 16 on Node"]
        direction TB
        proxy["proxy.ts<br/>CSP nonce + security headers<br/>admin session renewal"]
        pages["Server Components<br/>/ · /admin/login · /admin"]
        actions["Server Actions<br/>register · sign in · sign out"]
        dal["Data access layer · src/lib/api<br/>graphql() client, ApiError → form copy<br/>forwards visitor IP"]
        session["src/lib/session<br/>httpOnly cookie, requireAdmin()"]
    end

    subgraph api ["apps/api · NestJS + Apollo GraphQL"]
        direction LR
        guards["Guards<br/>AuthGuard: JWT + roles, deny by default<br/>GqlThrottlerGuard: rate limits + backoff"]
        resolvers["Resolvers<br/>register · serviceTypes · leads · lead<br/>login · renewToken · me · users"]
        services["Services<br/>Zod validation · DataLoader"]
        orm["Sequelize models"]
    end

    db[("Postgres 17<br/>leads · service_types<br/>lead_service_types · users")]

    visitor -- "pages, form POSTs (HTTPS)" --> proxy
    admin -- "pages, links, form POSTs (HTTPS)" --> proxy
    proxy --> pages & actions
    pages --> session
    actions --> session
    pages --> dal
    actions --> dal
    session --> dal
    dal -- "POST /graphql<br/>Bearer token · X-Forwarded-For<br/>(private network)" --> guards
    guards --> resolvers --> services --> orm --> db
```

- **The browser never calls the API.** Pages and Server Actions run on the Next server and call the API through `src/lib/api`, which keeps the API address and the admin's token off the client. The session is the API's JWT in an httpOnly cookie that browser JavaScript can't read.
- **Rate limits see real visitors.** The web forwards the visitor's IP (`X-Forwarded-For`), and the API trusts one proxy hop (`TRUST_PROXY=1`), so in production the API must be reachable only from the web server.
- **Every admin page and action calls `requireAdmin()`**, which asks the API (`me`) whose token it is. `proxy.ts` only keeps an active session going (sliding: 30 minutes idle, 8 hours at most); it never decides access.
- **Validation happens twice**: in the browser for instant feedback (nothing is sent until it passes), and in the API with Zod as the source of truth.

**Repository map**

| Path | What |
|---|---|
| `apps/web/src/app` | Routes: pages, Server Actions, error and 404 pages, `robots.ts`, `sitemap.ts` |
| `apps/web/src/components` | Atomic design: `atoms` → `molecules` → `organisms` → `templates` (ESLint enforces the direction) |
| `apps/web/src/lib` | Data access (`api/`), session, URL state, validation, security headers |
| `apps/web/e2e` | Playwright end-to-end tests (mobile and desktop, with axe) |
| `apps/api/src/{auth,users,leads,common}` | Nest modules: auth and guards, accounts, leads, shared errors and rate limiting |
| `apps/api/src/database` | Migrations (Umzug), migration runner, dev seed |
| `apps/api/test` | API end-to-end tests against real Postgres, and the HTTP smoke test |
| `apps/api/bruno` | A Bruno collection of every operation |
| `packages/validation` | Registration and sign-in rules (Zod), shared by the API and the web forms |
| `scripts/bootstrap-env.mjs` | Creates the local env files for `pnpm bootstrap` |

## Request flows

### A visitor registers

```mermaid
sequenceDiagram
    autonumber
    actor V as Visitor
    participant B as Browser
    participant W as Next server (apps/web)
    participant A as API (apps/api)
    participant D as Postgres

    V->>B: Fills in the form (each change saved to sessionStorage), presses Register
    B->>B: validateRegistration(): mistakes shown at once, nothing sent
    B->>V: "Thanks, you're registered" at once (useOptimistic)
    B->>W: Server Action registerAction (POST /)
    W->>A: mutation register(…) with X-Forwarded-For
    A->>A: Throttler (5/min per visitor, backoff) · Zod validation
    A->>D: INSERT lead + lead_service_types (one transaction)
    alt saved
        D-->>A: ok
        A-->>W: Lead
        W-->>B: success
        B->>B: Draft cleared, the confirmation stays
    else email already registered, rate limited, API down, connection lost
        A-->>W: CONFLICT / TOO_MANY_REQUESTS / error (or no answer)
        W-->>B: error with the typed values
        B->>V: Confirmation withdrawn: the form with the reason, focus on the problem
    end
```

- **Only the screen is fire and forget.** "Thanks" shows before the server answers, but it stays only when Postgres has committed the lead; anything else withdraws it and shows why, with what was typed. A form with mistakes never shows it.
- **A reload keeps what was typed**: the draft lives in `sessionStorage` (this tab only) until the server confirms, and is restored after hydration.
- Without JavaScript, the browser posts the form to the same Server Action and the page comes back with the result.

### An admin opens the dashboard

```mermaid
sequenceDiagram
    autonumber
    actor Ad as Admin
    participant B as Browser
    participant P as proxy.ts
    participant W as Next server
    participant A as API
    participant D as Postgres

    Ad->>B: Opens /admin?q=ada&sort=name_asc
    B->>P: GET /admin (cookie: session JWT)
    P->>P: New CSP nonce
    opt token has < 10 minutes left
        P->>A: renewToken
        A-->>P: fresh JWT (same sign-in time)
        Note over P,B: the fresh JWT goes back as an httpOnly Set-Cookie on this response
    end
    P->>W: render /admin
    W->>A: me (requireAdmin)
    alt no session, or not an admin
        W-->>B: redirect /admin/login?next=/admin?q=ada&sort=name_asc
    else admin
        par in parallel
            W->>A: serviceTypes
        and
            W->>A: leads(search: "ada", sort: NAME_ASC, limit, offset)
            A->>D: SELECT … ILIKE (trigram index) + services in one batch (DataLoader)
        end
        A-->>W: page of leads + total
        W-->>B: HTML with the nonce on every script
    end
```

## Reliability: known gaps (TODO)

Registration is saved when Postgres commits, and the visitor sees "Thanks" for good only after that. A registration the server accepted is never lost. These three gaps remain, where the visitor is misled or has to act. Gap 3 matters most; they're numbered in the order to build them, since 2 and 3 both need 1.

| # | Gap | What happens today | Why it matters |
|---|---|---|---|
| 1 | **Saved, but the answer is lost**: the API commits, then the web's call times out (`TIMEOUT_MS` in `src/lib/api/client.ts`), the network drops or Next restarts | The visitor sees "We couldn't send your registration" and presses Try again; the API answers `CONFLICT`, and the form says the email "has already registered interest" | The visitor is told two wrong things in a row, and retries can never be automatic, since a retry of a saved registration fails |
| 2 | **The browser never delivers it**: the tab is closed, or the connection drops, in the second after "Thanks" shows | Lost. The draft is in `sessionStorage`, which goes when the tab closes | The visitor saw a confirmation for a registration that doesn't exist, and nobody knows |
| 3 | **The API or Postgres is briefly down** | The visitor sees "couldn't send" with their details kept, and must try again later | Registrations during an outage depend on each visitor coming back; some won't |

- [ ] **1. Make register safe to retry (idempotency key).** The key is a hash of the submission's content, computed on the Next server: `HMAC-SHA256(secret, normalised values)`, or a UUID v5 with a secret namespace if the UUID format is wanted. It needs no state, so every path gets the same key without storing one: the direct call, automatic retries, the resend in 2 and the queue in 3.
  - **Normalise before hashing, exactly as the API does** (the shared rules in `packages/validation`): trim everything, lowercase the email, the mobile as digits only (`+61 412 345 678` → `0412345678`), services de-duplicated and sorted. Otherwise a trailing space or another mobile format gives a different key.
  - **Only on the server, with a secret key** (an env var). The same computation in the browser would ship the key in JavaScript, and anyone could compute the key of a guessed email and details: a guessable fingerprint of personal data.
  - **Identical details are the same submission; changed details are a new one.** The API stores the key on the lead (unique `submission_id` column, one migration). The same key again answers success with the existing lead; a new key with an email already registered still gets `CONFLICT`. So a visitor who corrects a typo and submits again is told the email is taken, instead of the correction being silently dropped. A key from the email alone would drop it: both submissions would share one key.
  - Then the web retries network and server errors automatically (a couple of times, short backoff) before showing "couldn't send".
- [ ] **2. Resend what the browser didn't deliver.** Keep the submitted values in `localStorage` (which outlives the tab) from submit until the server confirms, and send them again on the next visit; the server computes the same key from them. Needs 1, so a resend of a saved registration answers success, not `CONFLICT`. Only helps when the visitor comes back in the same browser.
- [ ] **3. Accept registrations while the API or Postgres is down.** A second durable store the Next server can reach on its own: Postgres can't cover its own outage, and neither can the API. An SQS FIFO queue fits one consumer at this volume with nothing to operate: when the API is unreachable, errors or times out, the Next server sends the registration to the queue and answers success; an API worker saves it once the API is back.

  | SQS field | Value | Why |
  |---|---|---|
  | `MessageDeduplicationId` | the content key from 1 | SQS drops a repeat of the same submission within 5 minutes (a retry, a double send); a corrected submission has a new key and goes through |
  | `MessageGroupId` | `HMAC(secret, normalised email)` | one email's submissions are processed in order, different emails in parallel, and the email itself never appears in queue metadata |

  The 5-minute window only catches quick repeats: the worker stays idempotent through the `submission_id` from 1, so a replay days later is still success. A save that fails comes back after the visibility timeout and moves to a dead-letter queue after a few tries; messages are kept up to 14 days. Kafka does the same with more to run (it keeps messages for a set time or size, 7 days by default, unless configured otherwise): worth it once registrations feed several systems. Decisions to make first: queue only as a fallback (keeps duplicate-email, rate-limit and field feedback) or every registration (one path, but that feedback moves or disappears); rate limiting on the queued path. Needs 1, so a registration that was saved before the timeout isn't saved twice.

For side effects of a registration later (confirmation email, CRM sync), use a transactional outbox rather than publishing from the API: an `outbox` row written in the lead's transaction, processed by a worker with retries. It can feed SQS, SNS or Kafka too.
